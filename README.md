...
## Теоретическое пояснение к структуре

---

## это критически важный момент для безопасности (МДК.09.03).**

Если вы просто скопируете всё содержимое папки `/project-root` (включая `/config`, `/src`, `.git`) внутрь `/public_html` на хостинге, вы совершите **грубейшую ошибку безопасности**.

### Почему это опасно?
Папка `public_html` (или `www`, `htdocs`) — это **Document Root**. Всё, что находится внутри неё, доступно любому человеку в интернете через браузер.

Если ваша структура выглядит так:
```text
/public_html
    /config
        db.php      <-- ДОСТУПЕН по ссылке site.ru/config/db.php
    /src
        User.php    <-- ДОСТУПЕН по ссылке site.ru/src/User.php
    index.php
```
Хакер (или преподаватель на защите) может напрямую скачать ваши файлы конфигурации или классов, если веб-сервер (Nginx/Apache) случайно перестанет обрабатывать PHP файлы как скрипты и отдаст их как текст. А папка `.git`, попавшая туда, позволит выкачать весь исходный код проекта.

---

### Как должно быть (Архитектура "Best Practice")

В профессиональной разработке (и для оценки «Отлично»):
1.  `/project-root` — это корень вашего проекта (весь репозиторий).
2.  `/public_html` (или просто `/public`) — это **вложенная** папка внутри корня.

#### Правильная структура на Хостинге (Beget / TimeWeb и др.):

На сервере это должно выглядеть так (уровень выше `public_html`):

```text
/home/u/user/          <-- Ваш домашний каталог на хостинге
    /site-core/        <-- (Бывший /src + /config). НЕ ДОСТУПНА из браузера!
        /config/db.php
        /models/User.php
        /templates/
    
    /site.ru/public_html/   <-- Только ТОЧКА ВХОДА и статика
        index.php      <-- Единственный PHP файл здесь
        /css/
        /js/
        /images/
```

#### Что менять в `index.php`?
В файле `index.php`, который лежит в `public_html`, вы подключаете файлы, выходя на уровень вверх:

```php
<?php
// index.php

// Выходим на уровень выше (..) и заходим в папку с ядром
require_once __DIR__ . '/../../site-core/config/db.php';
require_once __DIR__ . '/../../site-core/models/User.php';
...
### Компромиссный вариант (Если сложно переделать структуру)

Если вы (как новичок) всё же загрузили всё внутрь `public_html`, вы **ОБЯЗАНЫ** защитить папки с логикой.

Для этого в папках `/config`, `/src`, `/templates` создайте файл `.htaccess` со следующим содержимым:
...
**Это запретит прямой доступ к файлам через браузер, но PHP сможет их подключать через `require`.**

---

### Итог для семинара:
1.  **Локально:** Вы работаете в `/project-root`.
2.  **На хостинге:** Вы создаете отдельную папку для кода (недоступную из веба) и отдельную папку `public_html` только для `index.php` и картинок.
3.  **Критерий оценки:** Если я (методист) могу открыть `http://ваш-сайт.ru/config/db.php` и увидеть белый экран (или код) — это «двойка» за безопасность. Я должен увидеть ошибку **403 Forbidden**.
